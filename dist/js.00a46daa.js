...
exports.get_Details = exports.getRecipeData = exports.reuseFunt = void 0;
...
const get_Details = data => {
  ...
  return recipe; // const returnedData = fetchRecipeDetails(recipe);
...
class SearchTerm {
  ...
  async fetchData() {
    try {
      let response = await (0, _axios.default)("https://forkify-api.herokuapp.com/api/search?q=".concat(this.query)); // const savedResponse = getData(response);

      const savedResponse = (0, _Fetch.reuseFunt)(response);
...
},{"axios":"../node_modules/axios/index.js","./Fetch":"js/models/Fetch.js"}],"js/views/addLoader.js":[function(require,module,exports) {
"use strict";
...
const recipeDetails_DOM = recipe => {
  ...
  if (!creatElem) {
    ...
    EmptyDOM(_base.element.detailSection);
...
const new_data = async id => {
  try {
    const res = await (0, _axios.default)("https://forkify-api.herokuapp.com/api/get?rId=".concat(id));
    return res; // reuseFunt(res); 
    // get_Details(res) ;
  } catch (error) {
    throw error;
  }
...
_base.element.submitText.addEventListener('submit', e => {
  ...
  queryValue(); // CLEAR INPUT ON CLICK OF SUBMIT BUTTON
...
const queryValue = () => {
  if (_base.element.searchQuery.value.trim() == '') {
...
_base.element.dataList.addEventListener('click', e => {
  ...
  if (targetList) {
    awaitRecipeDetails(targetList); // ADD MEDIA QUERY FUNCTION

    mediaQuery.addListener(detailsLenght(mediaQuery)); // ADD LOADER AND FETCH DATA
  }
});
